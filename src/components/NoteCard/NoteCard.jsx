...
export function NoteCard(props){
  ...
  let {
    ...
    const handlePinnedClick=(args)=>{
        let tempNotes=notes.map(function(note){
            if(note.id===args.id){
    ...
    return (
        <div className="DisplayContainer">
            <div>{props.title} {props.isPinned ?  <RiUnpinLine  
            onClick={()=>handlePinnedClick(props)}
             className="NoteCardIcons noteIcons"
            /> : <BsPinFill onClick={()=>handlePinnedClick(props)}
            className="NoteCardIcons noteIcons"/>} 
             </div>
                <hr />
                <div className="noteRenderer">{props.note} 
                   <div className="NoteCardIcons ">{props.isArchived ? <MdOutlineUnarchive className="noteIcons"  onClick={()=>handleArchivedClick(props)}/>: <BiArchiveIn className="noteIcons"  onClick={()=>handleArchivedClick(props)}/>}<ImBin2 className="noteIcons"/></div>
                </div>
        </div>
    )
    }
